...
class MaxPathWeightWithTwoSidedConnectorsPart2Test {
    @ParameterizedTest
    @ArgumentsSource(ArgumentsProviderMaxPathWeightWithTwoSidedConnectors::class)
    fun `test findMaxPathWeightForLongestPath`(expected: Int, connectors: List<Connector>) =
        with(MaxPathWeightWithTwoSidedConnectorsPart2()) {
            assertEquals(expected, findMaxPathWeightForLongestPath(connectors))
        }
