...
class MaxSquareSumTest {
    ...
    fun `test findMaxSumOf3x3Square`(expected: MaxSquareSumResult, k: Int, gridWidth: Int, gridHeight: Int) =
        with(MaxSquareSum()) {
    ...
    private object ArgumentsProviders {
        class ArgumentsProviderMaxSumOf3x3Square : ArgumentsProvider {
            override fun provideArguments(p0: ExtensionContext?) = Stream.of(
                arguments(
                    named("expected", MaxSquareSumResult(sum = 29, x = 33, y = 45)),
                    named("k", 18), named("gridWidth", 300), named("gridHeight", 300)
                ),
                arguments(
                    named("expected", MaxSquareSumResult(sum = 30, x = 21, y = 61)),
