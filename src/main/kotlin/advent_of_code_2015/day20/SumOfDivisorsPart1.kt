...
/**
 * [Infinite Elves and Infinite Houses - Part 1](https://adventofcode.com/2015/day/20)
 *
 * For `i, j >= 1`, let
 * - `C_i = {i, 2 * i, 3 * i, ...}` be infinite cycles of all multiples of `i`
 ...
 */
class SumOfDivisorsPart1 {
    fun minNumberToReachSum(k: Int): Int {
...
private fun printResult(inputFileName: String) {
    ...
    val k = ceil(readInput(inputFileName) / 10.0).toInt()
    val solver = SumOfDivisorsPart1()
    println("Minimum number to reach sum $k: ${solver.minNumberToReachSum(k)}")
}
